...
class Exporter():
    def __init__(self, target_systemd_unit: str, logger: logging.Logger = None, log_level: str = "INFO", systemd_log_level: str = "INFO"):
        # set up our logger
        if not logger:
        ...
        if target_systemd_unit:
            ...
            logging.debug("Target systemd service provided, targeting _SYSTEMD_UNIT [%s]", target_systemd_unit)
    ...
    def log(self):
        ...
        if self._target_systemd_unit:
            self.logger.info("Logging events to systemd service [%s]", self._target_systemd_unit)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l", "--exported-log-level", dest="systemd_log_level", default=os.environ.get("EXPORTER_SYSTEMD_LOG_LEVEL", "INFO"), help="The log level to read systemd logs at; one of ALERT, CRIT, DEBUG, EMERG, ERR, INFO, LOG, WARNING")
    parser.add_argument("-t", "--target", "-t", dest="target_systemd_unit", default=os.environ.get("EXPORTER_SYSTEMD_TARGET"), help="The systemd unit to target")
    parser.add_argument("--log-level", dest="log_level", default=os.environ.get("EXPORTER_PYTHON_LOG_LEVEL", "INFO"), help="The **Python** log level to output application debug logs at")
